								...
								<?php    while($row = $fetch_data->fetch_assoc()) {?>
									<tr>
										...
										<td> <?php if($id == $row['therapist_id']){ ?>
											...
											<a href="<?php echo $path_obj->adminpath("schedule"); ?>/action/remove-schedule.php?id=<?php echo $row['id']; ?>" class="mt-2 remove_session btn btn-primary btn-sm">Remove</a>
										<?php }elseif($id == $admin_id){ ?><a href="#" data-id="<?php echo $row['id']; ?>" 
											
											class="btn btn-primary modal-edit btn-sm" id="edit-session-modal">Edit</a>
											<a href="<?php echo $path_obj->adminpath("schedule"); ?>/action/remove-schedule.php?id=<?php echo $row['id']; ?>" class="mt-2 remove_session btn btn-primary btn-sm">Remove</a><?php } ?>
										</td>
									</tr>
									<?php } ?>
